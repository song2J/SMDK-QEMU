...
static inline bool get_dip_policy(CacheLine* curr)
{
    return ((PolicySpecificData*)(curr->policy_specific_data))->if_bip;
}

static inline void set_dip_policy(CacheLine* curr, bool if_bip)
{
    ((PolicySpecificData*)(curr->policy_specific_data))->if_bip = if_bip;
}
...
static void cache_promote_line(CMMHCache *cc, uint64_t idx, CacheLine *curr)
{
    ...
    /* Phase3: Update dip list(if curr == lru_head(or lru_tail if MRU), do nothing) */
    if(get_dip_policy(curr) && g_random_double() < table->bip_alpha) {
        if(curr == *lru_tail_p)
            return;
        ...
        if(*lru_head_p == curr)
            *lru_head_p = get_next_lru_line(curr);
        ...
        set_prev_lru_line(curr, *lru_tail_p);
        set_next_lru_line(*lru_tail_p, curr);
        set_next_lru_line(curr, NULL);
        *lru_tail_p = curr;
    } else {
        if(curr == *lru_head_p)
            return;
...
static CacheLine* cache_access(CMMHCache *cc, uint64_t dpa, uint64_t *victim)
{
    ...
    *victim = table->lru_tail->dpa;
    ...
    table->psel = table->psel * (1.0 - table->dip_alpha) + (get_dip_policy(table->lru_tail) ? table->dip_alpha : 0.0);
    set_dip_policy(table->lru_tail, table->psel < 0.5);
...
void cmmh_cache_global_lru_init(CMMHCache *cc)
{
    ...
    cc->table = g_malloc0(sizeof(DIPCache));

    ((DIPCache*)cc->table)->heads = g_malloc0(sizeof(CacheLine*) * (1 << index_bits));
    ((DIPCache*)cc->table)->lru_head = NULL;
    ((DIPCache*)cc->table)->lru_tail = NULL;
    ((DIPCache*)cc->table)->dip_alpha = 1.0/20;
    ((DIPCache*)cc->table)->bip_alpha = 1.0/32;
    ((DIPCache*)cc->table)->psel = 1.0/2;

    CacheLine** heads = ((DIPCache*)cc->table)->heads;
    CacheLine** lru_head_p = &(((DIPCache*)cc->table)->lru_head);
    CacheLine** lru_tail_p = &(((DIPCache*)cc->table)->lru_tail);
    ...
    for(int i = 0; i < (1 << index_bits); i++) {
        ...
        for(int j = 0; j < num_tag; j++) {
            ...
            *lru_head_p = curr;
            ...
            set_dip_policy(curr, g_random_int_range(0, 2) == 1);
        }
    }
