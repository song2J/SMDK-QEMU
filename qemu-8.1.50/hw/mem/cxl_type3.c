...
static int ct3_build_cdat_table(CDATSubHeader ***cdat_table, void *priv)
{
    ...
    if (!ctd3->hostcmmh && !ct3d->hostpmem && !ct3d->hostvmem && !ct3d->dc.num_regions) {
        return 0;
    }

    if ((ct3d->hostcmmh || ct3d->hostpmem) && ct3d->hostvmem && ct3d->dc.host_dc) {
...
static void build_dvsecs(CXLType3Dev *ct3d)
{
    ...
    if (ct3d->hostvmem) {
        ...
        } else if (ct3d->hostcmmh) {
            range2_size_hi = ct3d->hostcmmh->size >> 32;
            range2_size_lo = (2 << 5) | (2 << 2) | 0x3 |
                             (ct3d->hostcmmh->size & 0xF0000000);
        ...
        range1_size_hi = ct3d->hostcmmh->size >> 32;
        range1_size_lo = (2 << 5) | (2 << 2) | 0x3 |
                         (ct3d->hostcmmh->size & 0xF0000000);
        if (ct3d->dc.host_dc) {
            range2_size_hi = ct3d->dc.host_dc->size >> 32;
...
static int cxl_create_dc_regions(CXLType3Dev *ct3d)
{
    ...
    if (ct3d->hostcmmh) {
        region_base += ct3d->hostcmmh->size;
    }
...
static bool cxl_setup_memory(CXLType3Dev *ct3d, Error **errp)
{
    ...
    if (!ct3d->hostmem && !ct3d->hostvmem && !ct3d->hostpmem && !ct3d->hostcmmh
            && !ct3d->dc.num_regions) {
        error_setg(errp, "at least one memdev property must be set");
        return false;
    } else if (ct3d->hostmem && (ct3d->hostpmem || ct3d->hostcmmh)) {
        error_setg(errp, "[memdev] cannot be used with new "
                         "[persistent-memdev] property");
        return false;
    } else if (ct3d->hostpmem && ct3d->hostcmmh) {
        error_setg(errp, "[cmm-hybrid-memdev] cannot be used with "
                         "[persistent-memdev] property");
        return false;
...
err_address_space_free:
    ...
    if (ct3d->hostcmmh) {
        address_space_destroy(&ct3d->hostcmmh_as);
    }
    if (ct3d->hostvmem) {
...
static int cxl_type3_hpa_to_as_and_dpa(CXLType3Dev *ct3d,
                                       ...
                                       uint64_t *dpa_offset)
{
    MemoryRegion *vmr = NULL, *pmr = NULL, *nmr = NULL, *dc_mr = NULL;
    uint64_t vmr_size = 0, pmr_size = 0, nmr_size = 0, dc_size = 0;
    ...
    if (ct3d->hostcmmh) {
        nmr = host_memory_backend_get_memory(ct3d->hostcmmh);
        nmr_size = memory_region_size(nmr);
    }
    if (ct3d->dc.host_dc) {
        dc_mr = host_memory_backend_get_memory(ct3d->dc.host_dc);
    ...
    if ((*dpa_offset >= vmr_size + pmr_size + nmr_size + dc_size) ||
       (*dpa_offset >= vmr_size + pmr_size + nmr_size && ct3d->dc.num_regions == 0)) {
        return -EINVAL;
    }
    ...
    if (*dpa_offset < vmr_size) {
        ...
        *as = &ct3d->hostpmem_as;
        *dpa_offset -= vmr_size;
    } else if (*dpa_offset < vmr_size + pmr_size + nmr_size) {
        *as = &ct3d->hostcmmh_as;
        ...
        *dpa_offset -= (vmr_size + pmr_size + nmr_size);
    }
...
static Property ct3_props[] = {
    ...
    DEFINE_PROP_LINK("volatile-memdev", CXLType3Dev, hostvmem,
                     TYPE_MEMORY_BACKEND, HostMemoryBackend *), 
    DEFINE_PROP_LINK("cmm-hybrid-memdev", CXLType3Dev, hostcmmh, /********* FYI: CMM-H Property **********/
                     TYPE_MEMORY_BACKEND, HostMemoryBackend *),
    ...
    DEFINE_PROP_INT32("secsz", CXLType3Dev.fc, bb_params.secsz, 512),
    DEFINE_PROP_INT32("secs_per_pg", CXLType3Dev.fc, bb_params.secs_per_pg, 8),
    DEFINE_PROP_INT32("pgs_per_blk", CXLType3Dev.fc, bb_params.pgs_per_blk, 256),
    DEFINE_PROP_INT32("blks_per_pl", CXLType3Dev.fc, bb_params.blks_per_pl, 256),
    DEFINE_PROP_INT32("pls_per_lun", CXLType3Dev.fc, bb_params.pls_per_lun, 1),
    DEFINE_PROP_INT32("luns_per_ch", CXLType3Dev.fc, bb_params.luns_per_ch, 8),
    DEFINE_PROP_INT32("nchs", CXLType3Dev.fc, bb_params.nchs, 8),
    DEFINE_PROP_INT32("pg_rd_lat", CXLType3Dev.fc, bb_params.pg_rd_lat, 40000),
    DEFINE_PROP_INT32("pg_wr_lat", CXLType3Dev.fc, bb_params.pg_wr_lat, 200000),
    DEFINE_PROP_INT32("blk_er_lat", CXLType3Dev.fc, bb_params.blk_er_lat, 2000000),
    DEFINE_PROP_INT32("ch_xfer_lat", CXLType3Dev.fc, bb_params.ch_xfer_lat, 0),
    DEFINE_PROP_INT32("gc_thres_pcent", CXLType3Dev.fc bb_params.gc_thres_pcent, 75),
    DEFINE_PROP_INT32("gc_thres_pcent_high", CXLType3Dev.fc, bb_params.gc_thres_pcent_high, 95),
...
static bool set_cacheline(CXLType3Dev *ct3d, uint64_t dpa_offset, uint8_t *data)
{
    ...
    if (dpa_offset + CXL_CACHE_LINE_SIZE >= vmr_size + pmr_size + nmr_size
